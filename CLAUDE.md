...
op 1). Über generischem `<a>`/`<button>` OHNE `data-cursor` → **Blob** (24, op 0.35) —
...
Token-Var. **Größen: 16 (Dot) / 24 (Blob) / 12 (Klick).** **Klick-Feedback:** `pointerdown`
→ `.cursor--press` skaliert die Form auf ABSOLUT 12px, egal aus welchem Zustand: Dot
`--cursor-press: 0.75` (16→12), Blob `calc(12/24)` (24→12). Transition `transform 0.1s`,
